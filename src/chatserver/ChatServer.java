...
public class ChatServer {
    ...
    /**
     * The print writers for specific client
     */
    private static HashMap<String, PrintWriter> writerList = new HashMap<String, PrintWriter>();
    ...
    private static class Handler extends Thread {
            ...
            try {
                ...
                while (true) {
                    ...
                    if(input.contains(">>")) {
                		...
                		//find reciver
                        for (String reciverName : names) {
                            if (reciverName.equals(reciver)) {
                                System.out.println("PRIVATE MESSAGE");
                                //write on sender
                                writerList.get(name).println("MESSAGE " + name + ": " + msg);
                                
                                //write on reciver
                                writerList.get(reciver).println("MESSAGE " + name + ": " + msg);
                                break;
                            }
                        }
                        
                	} else {
                		//boardcast a msg if its not for a specific client
        ...
        //Updates client list function
        public void updateClients() {
            String[] keys = writerList.keySet().toArray(new String[0]);
            String clientArray = "";
            ...
            for (PrintWriter writer : writers) {
                writer.println("CLIENTLIST" + clientArray);
            }
        }
    }
}